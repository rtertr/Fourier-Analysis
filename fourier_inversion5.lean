...
def banach.fourier_integral (f : E → ℂ ) (w : E) {L: E→ₗ[ℝ] E →ₗ[ℝ] ℝ}:= vector_fourier.fourier_integral fourier_char measure_theory.measure_space.volume L f w
...
theorem integral_gaussian_eq_one (b : ℝ) : ∫ x, exp (- real.pi * x^2) = 1 :=
begin
...
theorem integral_gaussian_eq_one (b : ℝ) : ∫ x, cexp (- real.pi * (x:ℂ)^2) = 1 :=
begin
  have h₁: 0< (real.pi:ℂ ).re, by {norm_cast, exact pi_pos},
  have h₂: (real.pi:ℂ) ≠ 0, by {norm_cast, exact real.pi_ne_zero,},
  have h₃:=integral_gaussian_complex h₁,
  rw [div_self h₂,one_cpow] at h₃,
  exact h₃,
end


theorem integral_gaussian_complex {b : ℂ} (hb : 0 < re b) :
  ∫ x:ℝ, cexp (-b * x^2) = (real.pi / b) ^ (1 / 2 : ℤ ) :=
begin
  ...
  { -- squares of both sides agree
    ...
    conv_lhs { rw ←zpow_one (↑real.pi / c)},
    rw ← zpow_add _ _ (div_ne_zero (of_real_ne_zero.mpr pi_ne_zero) (nv hc)),
    norm_num },
  { -- RHS doesn't vanish
    rw [ne.def, zpow_eq_zero_iff, not_and_distrib],
...
lemma deriv_complex_id {x:ℝ} : deriv (λ (y : ℝ), (y:ℂ)) x= 1 := has_deriv_at.deriv (has_deriv_at.of_real_comp (has_deriv_at_id x))



lemma deriv_complex_id_2: deriv (λ (y : ℝ), (y:ℂ ))= 1 :=
begin
  ext1 x, 
  refine has_deriv_at.deriv (has_deriv_at.of_real_comp (has_deriv_at_id x)),
end
...
/-lemma cont_diff.deriv2 {f:ℝ → ℂ } {m:ℕ} (hf : cont_diff ℝ ⊤ f):  cont_diff ℝ m (deriv f) :=
begin
  have hn: ↑m ≤ (⊤ : ℕ∞), by {refine le_of_lt _, exact with_top.coe_lt_top m,},
  refine cont_diff.of_le _ hn,
  refine cont_diff.iterate_deriv 1 hf,
end-/

lemma cont_diff.deriv_top {f:ℝ → ℂ } (hf : cont_diff ℝ ⊤ f):  cont_diff ℝ ⊤ (deriv f) :=
begin
  refine cont_diff.iterate_deriv 1 hf,
end
...
def schwartz_deriv (f: schwartz_map ℝ ℂ): schwartz_map ℝ ℂ :=
  { to_fun := (λ x : ℝ,  deriv f.to_fun x) ,
    ...
    begin
      ...
      rw [iterated_fderiv_eq_equiv_comp, iterated_deriv_eq_iterate, linear_isometry_equiv.norm_map,← iterated_deriv_eq_iterate] at h₁,
      exact h₁,
    end}



lemma iterated_deriv_add (f: schwartz_map ℝ  ℂ): ∀ (n m :ℕ), ∀ x:ℝ,  iterated_deriv n (iterated_deriv m f.to_fun) x = iterated_deriv (n+m) (f.to_fun) x :=
begin 
  intros n m,
...
lemma norm_iterated_fderiv_iterated_deriv {n m:ℕ } (f: schwartz_map ℝ  ℂ ) {x:ℝ }:  ‖iterated_fderiv ℝ n (iterated_deriv m f.to_fun) x‖ = ‖iterated_fderiv ℝ (n+m) (f.to_fun) x‖ :=
begin 
  have h₁:= iterated_deriv_add f n m x,
  rw [iterated_fderiv_eq_equiv_comp, linear_isometry_equiv.norm_map,iterated_fderiv_eq_equiv_comp, linear_isometry_equiv.norm_map],
  rw h₁,
end
...
def schwartz_iterated_deriv (m:ℕ) (f: schwartz_map ℝ ℂ): schwartz_map ℝ ℂ :=
  { to_fun := (λ x : ℝ, iterated_deriv m f.to_fun x) ,
    smooth' := 
    begin
      convert cont_diff.iterate_deriv m f.smooth',
...
lemma rapid_decay_x_mul_iterated_deriv_zero {k:ℕ} {f: schwartz_map ℝ  ℂ}: ∀ (m : ℕ), ∃ (C : ℝ), ∀ (x : ℝ), ‖x‖ ^ k * ‖iterated_fderiv ℝ 0 (λ (x : ℝ), ↑x * iterated_deriv m f.to_fun x) x‖ ≤ C :=
begin
  have h₁:=f.decay',
  intro m,
...
lemma cont_diff_deriv_x_mul_iterated_deriv {n m:ℕ} {f: schwartz_map ℝ  ℂ}: cont_diff ℝ ↑n (deriv (λ (y : ℝ), (y:ℂ )) * iterated_deriv m f.to_fun) :=
begin
  refine cont_diff.mul _ _,
...
lemma cont_diff_x_mul_deriv_iterated_deriv {n m:ℕ} {f: schwartz_map ℝ  ℂ}: cont_diff ℝ ↑n ((λ (y : ℝ), ↑y) * deriv (λ (y : ℝ), iterated_deriv m f.to_fun y)) :=
begin
  refine cont_diff.mul _ _,
  dsimp only,
  exact of_real_clm.cont_diff,
...
lemma deriv_mul_rewrite {m:ℕ } {f: schwartz_map ℝ  ℂ}: deriv (λ (x : ℝ), ↑x * iterated_deriv m f.to_fun x) = deriv (λ (y : ℝ), ↑y) * iterated_deriv m f.to_fun + (λ (y : ℝ), (y:ℂ )) * deriv (λ (y : ℝ), iterated_deriv m f.to_fun y) :=
begin
  ext1,
  rw deriv_mul _ _,  
  simp,
...
lemma rapid_decay_x_mul_iterated_deriv {f: schwartz_map ℝ  ℂ}  :∀ (k n m : ℕ), ∃ (C : ℝ), ∀ (x : ℝ ), ‖x‖ ^ k * ‖iterated_fderiv ℝ n (λ (x : ℝ ),(x:ℂ) *((iterated_deriv  m f.to_fun x):ℂ)) x‖ ≤ C:=
begin
  have h₁:=f.decay',
  intros k n,
  ...
  rw [iterated_fderiv_eq_equiv_comp,iterated_deriv_eq_iterate, linear_isometry_equiv.norm_map,← iterated_deriv_eq_iterate,iterated_deriv_succ',
  deriv_mul_rewrite,iterated_deriv_eq_equiv_comp,linear_isometry_equiv.norm_map,iterated_fderiv_add_apply,deriv_complex_id_2,one_mul],
...
lemma rapid_decay_x_mul_pow:∀ (j: ℕ), ∀ (f: schwartz_map ℝ  ℂ),∀ (k n : ℕ),∃ (C : ℝ), ∀ (x : ℝ ), ‖x‖ ^ k * ‖iterated_fderiv ℝ n (λ (x : ℝ ),(x:ℂ )^j* f.to_fun x) x‖ ≤ C:=
begin
  intros j,
...
-- # Proof needed, in the book
lemma translation_invariance (h: ℝ) (f:ℝ → ℂ  ):  ∫ (y : ℝ), f (y-h) = ∫ (y : ℝ),  f y := 
begin
...
lemma proposition_1_2_i (y h:ℝ) (f: schwartz_map ℝ ℂ): real.fourier_integral (λ (y : ℝ), f.to_fun (y+h)) y =cexp (↑(2 * real.pi * y * h) * I) * real.fourier_integral f.to_fun y :=
begin
...
lemma integration_by_parts (g f: ℝ → ℂ ) (N:ℝ ) (hgdiff: differentiable ℝ g) (hfdiff: differentiable ℝ f) (hgcont: continuous (λ (x : ℝ), deriv g x)) (hfcont:continuous (λ (x : ℝ), deriv f x)): 
...
lemma schwartz_integration_by_parts (f: schwartz_map ℝ ℂ ){x:ℝ } {N:ℝ  }: ∫ (v : ℝ) in -N..N, (λ v, cexp (↑((-2) * real.pi * v * x) * I)) v * deriv f.to_fun v =  (λ v, cexp (↑((-2) * real.pi * v * x) * I)) N  * f.to_fun N  - (λ v, cexp (↑((-2) * real.pi * v * x) * I)) (-N)  * f.to_fun (-N) -∫ (v : ℝ) in -N .. N, ( deriv (λy, cexp (↑((-2) * real.pi * y * x) * I)) v) * f.to_fun v :=
begin
...
lemma rewrite_tendsto (f: schwartz_map ℝ ℂ) {x:ℝ }: tendsto (λn:ℝ ,∫ (v :ℝ  ) in -n..n,cexp (↑((-2) * real.pi * v * x) * I) • deriv f.to_fun v) (filter.at_top) (nhds (∫ (y : ℝ), (cexp (↑((-2) * real.pi * y * x) * I) • f.to_fun y) • (2 * ↑real.pi * I * ↑x))) ↔ tendsto (λn:ℝ ,(cexp (↑((-2) * real.pi * ↑n * x) * I) * f.to_fun ↑n - cexp (↑((-2) * real.pi * -↑n * x) * I) * f.to_fun (-↑n) - ∫ (y : ℝ) in -↑n..↑n, deriv (λ (y : ℝ), cexp (↑((-2) * real.pi * y * x) * I)) y * f.to_fun y )) (filter.at_top) (nhds (∫ (y : ℝ), (0 + 0 + cexp (↑((-2) * real.pi * y * x) * I) • f.to_fun y) • (2 * ↑real.pi * I * ↑x))):=
begin
...
lemma nonneg_expression1 (f: schwartz_map ℝ ℂ): ∀ x:ℝ,  0 ≤ ‖f.to_fun x‖ * (1 + ‖x‖ ^ 2) :=
begin 
  intro x,
  positivity,
end

lemma nonneg_expression2: ∀ x:ℝ,  0 <  (1 + ‖x‖ ^ 2) :=
begin 
  intro x,
  positivity,
end

lemma integral_schwartz_le_integral_moderate_decrease {f: schwartz_map ℝ ℂ} {i C:ℝ} (h:∀ (x : ℝ), ‖f.to_fun x‖ ≤ C / (1 + ‖x‖ ^ 2)): 
∫ (x : ℝ) in ((λy:ℝ ,-‖y‖) i)  ..((λy:ℝ ,‖y‖) i), ‖f.to_fun x‖ ≤ ∫ (x : ℝ) in ((λy:ℝ ,-‖y‖) i)..((λy:ℝ ,‖y‖) i), (C)/ (1+‖x‖^2):=
begin
  refine interval_integral.integral_mono _ _ _ _, 
  have h₁: (λy:ℝ,-‖y‖) i ≤ (λy:ℝ,‖y‖) i, by {simp only [neg_le_self_iff],exact norm_nonneg _,},
  refine h₁,
...
lemma integral_schwartz_norm_bounded {f: schwartz_map ℝ ℂ} (l:filter ℝ  ):∃ I:ℝ,  ∀ᶠ (i : ℝ  ) in l, ∫ (x : ℝ) in (λy:ℝ ,-‖y‖) i..(λy:ℝ ,‖y‖) i, ‖f.to_fun x‖ ≤ I:=
begin
  set a:= (λy:ℝ ,-‖y‖) with ha,
  set b:= (λy:ℝ ,‖y‖) with hb,
...
lemma integrable_schwartz_map (f: schwartz_map ℝ ℂ): measure_theory.integrable (λ (v : ℝ), f.to_fun v) measure_theory.measure_space.volume:=
begin  
...
lemma n_top_integral (f: ℝ→ ℂ) (hf: measure_theory.integrable (λ (v : ℝ), f v) measure_theory.measure_space.volume):
...
lemma n_top_integral_schwartz (f: schwartz_map ℝ ℂ):tendsto (λ (n : ℝ), ∫ (v : ℝ) in -n..n, f.to_fun v) at_top (nhds (∫ (v : ℝ), f.to_fun v)) :=
begin
...
lemma deriv_exp_neg_complex_mul  (a : ℂ ):  ∀(x:ℝ), deriv (λ (x : ℝ), complex.exp (a*x)) x = (λ (x : ℝ  ), (a) * complex.exp (a*x)) x:=
begin
  intro x,
...
lemma norm_deriv_rw {h x:ℝ }: ‖deriv (λ (y : ℝ), cexp (-(2 * ↑real.pi * ↑y * ↑h * I))) x‖ = ‖deriv (λ (y : ℝ), cexp ((↑((-2) * real.pi * h) * I)* y)) x‖:=
begin 
...
lemma deriv_rw {h:ℝ }: deriv (λ (y : ℝ), cexp (-(2 * ↑real.pi * ↑y * ↑h * I))) = deriv (λ (y : ℝ), cexp ((↑((-2) * real.pi * h) * I)* y)) :=
begin 
  congr,ext1 y,repeat {rw ← neg_mul,}, rw [mul_comm _ I, mul_comm _ I], repeat {rw ← mul_assoc,},repeat {rw mul_assoc,},
  rw mul_comm (y:ℂ) _,  rw ← mul_assoc _ (h:ℂ) _, norm_cast, repeat {rw ← mul_assoc,},repeat {rw mul_assoc,},
end

lemma deriv_continous {h:ℝ }: continuous (λ (v : ℝ), deriv (λ (y : ℝ), cexp (↑((-2) * real.pi * y * h) * I)) v):=
begin
  simp only [neg_mul, of_real_neg, of_real_mul, of_real_bit0, of_real_one],
  rw [deriv_rw], 
...
lemma tendsto_const_div_at_top_nhds_0 (C : ℝ) : tendsto (λ n : ℝ  , C / n) at_top (nhds 0) :=
begin
  cases le_or_lt  (|C|) 0,
...
lemma limit_N_pos  (f: schwartz_map ℝ ℂ) {x:ℝ } : tendsto (λ (n : ℝ), cexp (↑((-2) * real.pi * ↑n * x) * I) * f.to_fun n) at_top (nhds (0)):=
begin
  have h₂:=f.decay', 
  specialize h₂ 1,
  specialize h₂ 0,
...
lemma limit_N_neg  (f: schwartz_map ℝ ℂ) {x:ℝ } : tendsto (λ (n : ℝ),cexp (↑((-2) * real.pi * -↑n * x) * I) * f.to_fun (-n)) at_top (nhds (0)):=
begin
  have h₂:=f.decay', 
  specialize h₂ 1,
  specialize h₂ 0,
  ...
  rw sub_zero,
  rw norm_mul,
  rw complex.norm_exp_of_real_mul_I,
  rw one_mul,
  specialize h₂ (-n),
  rw norm_neg at h₂,
  have h₂: ‖n‖ ^ 1 * ‖iterated_fderiv ℝ 0 f.to_fun (-n)‖ ≤ ‖C‖, by refine le_trans h₂ (real.le_norm_self _),
...
lemma hypothesis3 {f: schwartz_map ℝ ℂ} {x:ℝ }: tendsto (λn:ℝ ,∫ (v :ℝ  ) in -n..n,cexp (↑((-2) * real.pi * v * x) * I) • (deriv f.to_fun v)) (filter.at_top) (nhds (∫ (y : ℝ), ( cexp (↑((-2) * real.pi * y * x) * I) • f.to_fun y) • (2 * ↑real.pi * I * ↑x))):=
begin
...
lemma proposition_1_2_iv (f: schwartz_map ℝ ℂ): ∀ (x:ℝ ),real.fourier_integral (deriv f.to_fun) (x)  = 2*real.pi*I*x * real.fourier_integral f.to_fun (x):= 
begin
  intro x,
...
lemma proposition_1_2_iv_iterated (n:ℕ)  (f: schwartz_map ℝ ℂ) : ∀ (x:ℝ ), real.fourier_integral (iterated_deriv n f.to_fun) (x) = (2*real.pi*I*x)^n * real.fourier_integral f.to_fun (x) :=
begin
  induction n with n hn,
  intro x,
...
lemma rewrite1 {x:ℝ } {n:ℕ } {f: schwartz_map ℝ ℂ} : fourier_integral f.to_fun x * ↑x ^ n * (2 * I * ↑real.pi) ^ n * (2 * I * ↑real.pi)⁻¹ ^ n = fourier_integral f.to_fun x * ↑x ^ n:=
begin
...
lemma proposition_1_2_iv_iterated_lemma (n:ℕ)  (f: schwartz_map ℝ ℂ) : ∀ (x:ℝ ), (1/(2*(real.pi:ℂ )*I))^n*real.fourier_integral (iterated_deriv n f.to_fun) (x) = (x)^n * real.fourier_integral f.to_fun (x) :=
begin
  ...
  have h₃: (1/(2 * ↑real.pi * I))^n ≠ 0, 
    ...
    norm_cast,
    exact real.pi_ne_zero,},
  have h₄: (1 / (2 * ↑real.pi * I)) ^ n * ((2 * ↑real.pi * I) ^ n * ↑x ^ n * fourier_integral f.to_fun x) =   ↑x ^ n * fourier_integral f.to_fun x, by {ring_nf, exact rewrite1,},
...
lemma deriv_exp_neg_sq (x : ℝ) (a : ℝ) :  deriv (λ (x : ℝ), exp (-a*x^2)) x = (λ (x : ℝ), (-2*a*x) * exp (-a*x^2)) x:=
begin
  rw [deriv.comp, real.deriv_exp],
  simp only [neg_mul, deriv.neg', deriv_const_mul_field', deriv_pow'', differentiable_at_id', nat.cast_bit0, algebra_map.coe_one,
  pow_one, deriv_id'', mul_one, mul_neg, neg_inj],
  rw [mul_comm, ← mul_assoc, mul_comm 2 a],
  exact differentiable_at_exp,
  simp only [differentiable_at.mul, differentiable_at_const, differentiable_at.pow, differentiable_at_id'],
end
...
lemma fourier_transform_eq_gaussian2 : ∀ x:ℝ, real.fourier_integral gaussian_complex_pi.to_fun x =gaussian_complex_pi.to_fun x:=
begin
  intro x,
...
lemma fourier_transform_eq_gaussian {x:ℝ}: real.fourier_integral gaussian_complex_pi.to_fun =gaussian_complex_pi.to_fun:=
begin
  have h₁:= fourier_transform_eq_gaussian2,
  exact funext h₁,
end

theorem fourier_transform_gaussian (x : ℝ) : ∃ (f : ℝ → ℂ ), f x = real.fourier_integral f x :=
begin
...
lemma rewrite_imaginary_part {x v:ℝ}: ↑((-2) * real.pi * x * v) * I = -((2:ℂ) * (real.pi:ℂ ) * (v:ℂ) * (x:ℂ ) * I) :=
begin
  repeat {rw ← neg_mul,},
  norm_cast,
  rw mul_assoc _ v x,
...
lemma step1 {f g: schwartz_map ℝ  ℂ}: ∀ (x : ℝ), measure_theory.integrable ((λ (y x : ℝ), f.to_fun y * g.to_fun x * cexp (↑((-2) * real.pi * y * x) * I)) x) measure_theory.measure_space.volume :=
begin
  intro x,
...
lemma rewrite_functin4 {f g: schwartz_map ℝ  ℂ}: (λ (x : ℝ), ∫ (y : ℝ), ‖f.to_fun x * g.to_fun y * cexp (↑((-2) * real.pi * x * y) * I)‖) = (λ (x : ℝ), (∫ (y : ℝ), ‖g.to_fun y‖)*(λ (x : ℝ),‖f.to_fun x‖) x ):=
begin
  ext1 w,
...
lemma step2 {f g: schwartz_map ℝ  ℂ}: measure_theory.integrable (λ (x : ℝ), ∫ (y : ℝ), ‖(λ (y x : ℝ), f.to_fun y * g.to_fun x * cexp (↑((-2) * real.pi * y * x) * I)) x y‖) measure_theory.measure_space.volume:=
begin
  dsimp only,
  rw rewrite_functin4,
  refine measure_theory.integrable.const_mul _ _,
...
/-lemma continuous_uncurry 
  {f : ℝ  → ℝ  → ℂ } (ha : ∀ a, continuous (f a)) 
  (hb : ∀ a, ∀ b, continuous (f a b))
...
-- # medium
lemma step3 {f g: schwartz_map ℝ  ℂ}: continuous (function.uncurry (λ (y x : ℝ), f.to_fun y * g.to_fun x * cexp (↑((-2) * real.pi * y * x) * I))):=
begin
  suggest,
  sorry,
end

lemma multiplication_formula : ∀  (f g: schwartz_map ℝ  ℂ ), ∫ (x : ℝ), (f.to_fun x) * (real.fourier_integral g.to_fun x) = ∫ (y : ℝ), (real.fourier_integral f.to_fun y) * (g.to_fun y):= 
begin
  intros f g,
...
lemma deriv_rw2 {h :ℝ }: deriv (λ (y : ℝ), cexp (-(2 * ↑real.pi * ↑h * ↑y * I)))  = deriv (λ (y : ℝ), cexp ((↑((-2) * real.pi * h) * I)* y)) :=
begin 
...
lemma integral_deriv_rw (f: schwartz_map ℝ ℂ) {x:ℝ}: (λ (n : ℝ), ∫ (v : ℝ) in -n..n, deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x) = λ (n : ℝ), (∫ (v : ℝ) in -n..n,  (λ (w : ℝ), -(2 * ↑real.pi * ↑v * I) * cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x):=
begin
  ext1 n, congr, ext1 x, rw [deriv_mul_const,deriv_rw2,deriv_exp_neg_complex_mul], dsimp, rw neg_mul, congr,
...
lemma functions_equal_1 {f: schwartz_map ℝ ℂ} {x:ℝ}: (λ (v : ℝ), (λ (v : ℝ), ↑v * I * cexp ((-2) * ↑real.pi * ↑v * ↑x * I) * f.to_fun v) v) = λ (v : ℝ), I * (λ (v : ℝ), cexp ((-2) * ↑real.pi * ↑v * ↑x * I) * (↑v * f.to_fun v)) v:=
begin
  ext1 v,  dsimp,
  rw [mul_comm _ I,mul_assoc I _ _, mul_assoc I _ _, mul_comm (v:ℂ) _, mul_assoc _ (v:ℂ) _],
end

lemma functions_equal_2 {f: schwartz_map ℝ ℂ} {x:ℝ}: (λ (v : ℝ), (-2) * (real.pi:ℂ) * ↑v * I * cexp ((-2) * (real.pi:ℂ) * ↑v * ↑x * I) * (f.to_fun v)) =(λ (v : ℝ), ((-2) * (real.pi:ℂ)) * (λ (v : ℝ), ↑v * I * cexp ((-2) * (real.pi:ℂ) * ↑v * ↑x * I) * f.to_fun v) v):=
begin
  ext1 v, dsimp, repeat{rw ←mul_assoc,},
end

lemma rewrite5 {a:ℂ}: (λ (x : ℝ),cexp (-(2 * ↑real.pi * ↑a * ↑x * I))) = (λ (x : ℝ),cexp (-(2 * ↑real.pi * ↑a  * I) * ↑x)):=
begin
  ext1 x,
  congr,
  repeat {rw ← neg_mul,},
...
lemma ae_strongly_measurable_deriv_cexp_mul_schwartz {x:ℝ } {f: schwartz_map ℝ ℂ}:measure_theory.ae_strongly_measurable (λ (a : ℝ), deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x) measure_theory.measure_space.volume:=
begin
...
lemma ae_strongly_measurable_cexp_mul_schwartz {x:ℝ } {f: schwartz_map ℝ ℂ}:  ∀ᶠ (x : ℝ ) in nhds x, measure_theory.ae_strongly_measurable ((λw:ℝ,(λv:ℝ,cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v)) x) measure_theory.measure_space.volume:=
begin
  refine eventually_of_forall _, 
  intro x,
  refine continuous.ae_strongly_measurable _,
...
lemma rewrite3 {C:ℝ} {n:ℕ}: (λ (x : ℝ), ‖(-(2:ℂ)) ^ n‖ • ‖I ^ n‖ • ‖(real.pi:ℂ) ^ n‖ * (‖C‖ / (1 + ‖x‖ ^ 2)))  =(λ (x : ℝ), ((‖(-(2:ℂ )) ^ n‖ • ‖I ^ n‖ • ‖(real.pi:ℂ ) ^ n‖ * ‖C‖) / (1 + ‖x‖ ^ 2))):=
begin
  ext1 x, rw div_eq_mul_one_div, nth_rewrite_rhs 0 div_eq_mul_one_div, repeat {rw ← mul_assoc,},
end 

lemma integral_moderate_decrease_bounded {C:ℝ} (l:filter ℝ  ):∃ I:ℝ,  ∀ᶠ (i : ℝ  ) in l, ∫ (x : ℝ) in (λy:ℝ ,-‖y‖) i..(λy:ℝ ,‖y‖) i, ‖(λ (x : ℝ), C / (1 + ‖x‖ ^ 2)) x‖ ≤ I:=
begin
  set a:= (λy:ℝ ,-‖y‖) with ha,
  set b:= (λy:ℝ ,‖y‖) with hb,
...
lemma rewrite_bound {y C₁:ℝ } (h: 0 ≤ C₁ ): (2 * (|real.pi|) * ‖y‖) * ((C₁)/(1+ ‖y‖*(1+ ‖y‖^2))) ≤ C₁ * 2 * (|real.pi| / (1 + ‖y‖ ^ 2)):=
begin
...
lemma has_deriv_fourier_transform  {x:ℝ } {f: schwartz_map ℝ ℂ}: has_deriv_at (λ (n : ℝ), ∫ (v : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑n * I)) * f.to_fun v) (∫ (a : ℝ), deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x) x:=
begin
...
lemma cont_diff_pow_mul_schwartz {n:ℕ} {f:schwartz_map ℝ ℂ}: cont_diff ℝ ⊤ (λ (x : ℝ), (↑x) ^ n * f.to_fun x):=
begin
...
lemma cont_diff_const_pow_mul_schwartz {n:ℕ} {f:schwartz_map ℝ ℂ} {a : ℂ}: cont_diff ℝ ⊤ (λ (x : ℝ), ( a * ↑x) ^ n * f.to_fun x):=
begin
...
lemma iterated_deriv_cexp_eq_mul_pow_cexp {f: schwartz_map ℝ ℂ} {n:ℕ} {v:ℝ}:∀x:ℝ,  iterated_deriv n (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x = (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I))* (-2* ↑real.pi*I*v)^n* f.to_fun v) x:=
begin
  induction n with n hn, 
  intro x,
...
lemma has_iterated_deriv_fourier_transform  {x:ℝ } {f: schwartz_map ℝ ℂ}: has_deriv_at (λ (n : ℝ), ∫ (v : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑n * I)) * f.to_fun v) (∫ (a : ℝ), deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x) x:=
begin
  have h₁:  ∀ᶠ (x : ℝ ) in nhds x, measure_theory.ae_strongly_measurable ((λw:ℝ,(λv:ℝ,cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v)) x) measure_theory.measure_space.volume,
    refine ae_strongly_measurable_cexp_mul_schwartz,
  have h₂:  measure_theory.ae_strongly_measurable (λ (a : ℝ), deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x) measure_theory.measure_space.volume, 
...
lemma integral_deriv_rw2 (f: schwartz_map ℝ ℂ) {x:ℝ}:  ∫ (v : ℝ), deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x =  (∫ (v : ℝ),  (λ (w : ℝ), -(2 * ↑real.pi * ↑v * I) * cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x):=
begin
  congr, ext1 x, rw [deriv_mul_const,deriv_rw2,deriv_exp_neg_complex_mul], dsimp, rw neg_mul, congr,
  repeat {rw ← neg_mul,},congr,norm_cast, repeat {rw ← mul_assoc,}, repeat {rw ← neg_mul,},
  repeat {rw ← neg_mul,}, rw mul_comm _ I, rw mul_comm _ I, rw mul_assoc I _ _, 
  congr,norm_cast, 
  refine differentiable.differentiable_at _,
...
lemma proposition_1_2_v {x :ℝ } (f: schwartz_map ℝ ℂ): real.fourier_integral (λ (x : ℝ), ((-2*real.pi*I*x):ℂ )*(f.to_fun x)) (x) = deriv (real.fourier_integral f.to_fun) x :=
begin 
  rw fourier_integral_eq_integral_exp_smul,
  rw [fourier_integral_eq_integral_exp_smul_funext, leibniz_rule, integral_deriv_rw2],
...
begin
  ...
  simp_rw rewrite3,
  refine integrable_moderate_decrease _,
  have h₃: tendsto (λn:ℝ, ∫ (v : ℝ) in -n .. n, (λx:ℝ, ‖C‖ / (1 + ‖x‖ ^ 2)) v) at_top (nhds(∫ (x : ℝ), (λx:ℝ, ‖C‖ / (1 + ‖x‖ ^ 2)) x)),
    refine n_top_integral_real (λx:ℝ, ‖C‖ / (1 + ‖x‖ ^ 2)) _,
    refine integrable_moderate_decrease _,
...
begin
  ...
  rw h₅ x,
  --have h₂:= fourier_integral_eq_integral_exp_smul ((λ (x : ℝ), ((-2) * ↑real.pi * I * ↑x) ^ n * f.to_fun x)),
  --have h₃:= funext h₂,
  --simp at h₃,
...
lemma ae_strongly_measurable_cexp_mul_schwartz2 {x:ℝ } (f: schwartz_map ℝ ℂ):  ∀ᶠ (x : ℝ ) in nhds x, measure_theory.ae_strongly_measurable ((λw:ℝ,(λv:ℝ,cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v)) x) measure_theory.measure_space.volume:=
begin
  refine eventually_of_forall _, 
  intro x,
  refine continuous.ae_strongly_measurable _,
...
lemma ae_strongly_measurable_deriv_cexp_mul_schwartz2 {x:ℝ } (f: schwartz_map ℝ ℂ) :measure_theory.ae_strongly_measurable (λ (a : ℝ), deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x) measure_theory.measure_space.volume:=
begin
  refine continuous.ae_strongly_measurable _,
  simp only [deriv_mul_const_field'],
  refine continuous.mul _ (schwartz_map.continuous f),
...
lemma integrable_iterated_deriv_cexp_mul {f: schwartz_map ℝ ℂ} {n:ℕ} {x v:ℝ }: measure_theory.integrable (λ (v : ℝ), iterated_deriv n (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x) measure_theory.measure_space.volume:=
begin
...
lemma deriv_cexp_mul_schwartz_moderate_decrease_const_pow_mul_schwartz {x:ℝ } {f: schwartz_map ℝ ℂ} {n:ℕ}:∃ (C : ℝ), ∀ᵐ (a : ℝ), ∀ (y : ℝ), y ∈ metric.ball x (‖C‖ + 1) → ‖iterated_deriv (n + 1) (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) y‖ ≤ C / (1 + ‖a‖ ^ 2):=
begin
  ...
  rw mul_assoc _ _ ((const_pow_mul_schwartz_map n f).to_fun a),
  rw mul_assoc _ _ (f.to_fun _),
  congr,
  repeat {rw ←neg_mul,},
  ring_nf,
...
lemma iterated_deriv_eq_const_pow_mul_schwartz_map {f: schwartz_map ℝ ℂ} {n : ℕ} {a: ℝ} : (λ (x : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑x * I)) * (const_pow_mul_schwartz_map n f).to_fun a) = iterated_deriv n (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a):=
begin
...
lemma deriv_cexp_mul_const_pow_mul_schwartz_map {f: schwartz_map ℝ ℂ} {n : ℕ} {a w: ℝ}: deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * (const_pow_mul_schwartz_map n f).to_fun a) w = (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * ((-2) * ↑real.pi * I * ↑a) ^ (n + 1) * f.to_fun a) w:=
begin
  simp only [deriv_mul_const_field'],
  have h₄:deriv (λ (x : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑x * I))) w= deriv (λ (x : ℝ), cexp (-(2 * ↑real.pi *I *  ↑a)* ↑x )) w, by {congr,ext1,ring_nf,},
...
lemma deriv_cexp_mul_const_pow_mul_schwartz_map_eq_iterated_deriv_succ {f: schwartz_map ℝ ℂ} {n : ℕ} {a w: ℝ} : deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * (const_pow_mul_schwartz_map n f).to_fun a) w = iterated_deriv (n + 1) (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) w:=
begin
  rw iterated_deriv_cexp_eq_mul_pow_cexp,
  refine deriv_cexp_mul_const_pow_mul_schwartz_map,
end

lemma metric_ball_has_deriv_at_iterated_deriv {f: schwartz_map ℝ ℂ} {n : ℕ} {x C: ℝ} : ∀ᵐ (a : ℝ), ∀ (x_1 : ℝ), x_1 ∈ metric.ball x (‖C‖ + 1) → has_deriv_at (λ (x : ℝ), iterated_deriv n (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x) (iterated_deriv (n + 1) (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x_1) x_1:=
begin
...
has_deriv_at (iterated_deriv n (λ (w : ℝ), ∫ (v : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v))  ((λx:ℝ,(∫ (v : ℝ), iterated_deriv (n+1) (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x)) x) x:=
begin
...
lemma continous_const_pow_mul_schwartz_map {h:ℝ } (g: schwartz_map ℝ  ℂ ): measure_theory.ae_strongly_measurable (λ (x : ℝ), 𝓕 g.to_fun x * (λ (x : ℝ), cexp (-↑real.pi * ↑h * ↑‖x‖ ^ 2)) x) measure_theory.measure_space.volume:=
begin
  refine continuous.ae_strongly_measurable _,
  refine continuous.mul _ _,
...
-- # SORRY-FREE (just not in this document)
...
lemma one_div_le_one_div_pow_2 {x:ℝ }:
 1/ (1+‖x‖^2)^2 ≤ 1/ (1+‖x‖^2):=
begin
...
lemma tendsto_integral_one_div_one_add_sq_real_pi:  tendsto (λn:ℝ, ∫ (x : ℝ) in -n..n, (λ (x : ℝ), 1 / (1 + x ^ 2)) x) at_top (nhds (real.pi / 2 + real.pi / 2)):=
begin
...
lemma exist_integral_moderate_decrease_bounded (C:ℝ) : ∃ I:ℝ , ∫ (x : ℝ), (λ (x : ℝ), ‖C‖ / (1 + ‖x‖ ^ 2)) x ≤ I:=
begin
  ...
  have h₃:= n_top_integral_real (λx:ℝ,(λ (x : ℝ), 1 / (1 + ‖x‖ ^ 2)) x) (integrable_moderate_decrease _),
  dsimp only at h₃,
  convert h₃,
  rw ←zero_add (0:ℝ), 
...
begin
  --ve h₀: ∃ C:ℝ, ∀ x:ℝ, ∀ n:ℝ, ∀ᵐ (x_1 : ℝ), ‖(λ (v : ℝ), cexp (↑((-2) * real.pi * v * x) * I) • g.to_fun v * cexp (-↑real.pi * ↑n * ↑‖x‖ ^ 2)) x_1‖ ≤ ‖C‖ / (1 + ‖x_1‖ ^ 2), sorry,
...
lemma converges_proper (g: schwartz_map ℝ  ℂ ): tendsto (λ δ:ℝ  ,  ∫ (x : ℝ), 𝓕 g.to_fun x * ((λ x : ℝ , complex.exp (-real.pi *δ* ‖x‖^2)) x)) (nhds_within 0 (set.Ioi 0)) (nhds (∫ (x : ℝ), 𝓕 g.to_fun x)) :=
begin
...
lemma cexp_bound2 (h:ℝ) {a:ℝ} : 
∃ (C : ℝ), ∀ (x : ℝ), ‖cexp (↑((a) * x * h) * I)‖ ≤ C:=
begin
  use 1,  intro x,  rw [complex.norm_exp_of_real_mul_I _],
end

lemma continuous_cexp2 {x a:ℝ } : continuous (λ (v : ℝ), cexp ((a:ℂ) * ↑v * ↑x * I)):=
begin
  refine continuous.cexp  _,
  refine continuous.mul _ continuous_const,
  refine continuous.mul _ continuous_const,
...
begin
  ...
  repeat {rw ←neg_mul,},
  norm_cast,
  convert continuous_cexp2,
end

lemma cexp_const_mul_smul {δ x:ℝ} {f: schwartz_map ℝ ℂ}: (λ (v : ℝ), cexp (-(2 * ↑real.pi * ↑(x / δ) * ↑v * I)) • f.to_fun v) = λ (v : ℝ), cexp (↑(((-2:ℤ):ℝ) * real.pi * (1 / δ)) * ↑v * ↑x * I) * f.to_fun v:=
begin
  ext1 v,
  rw smul_eq_mul,
  repeat {rw ← neg_mul, },
...
lemma tendsto2{δ x:ℝ} {f: schwartz_map ℝ ℂ} (h: 0<δ): tendsto (λ (n : ℝ), (1 / (δ:ℂ )) * ∫ (v : ℝ) in (δ * -n)..(δ * n), (λ (w : ℝ), cexp (↑((-(2:ℝ)) * real.pi * 1 / δ * w * x) * I) • f.to_fun w) (v)) at_top (nhds (1 / ↑δ * ∫ (v : ℝ), cexp (↑((-(2:ℝ )) * real.pi * v * (x / δ)) * I) • f.to_fun v)):=
begin
  simp_rw rewrite_imaginary_part,
  refine filter.tendsto.const_mul _ _,
...
lemma proposition_1_2_iii {x δ:ℝ } (f: schwartz_map ℝ ℂ) (h: 0<δ): real.fourier_integral (λ (y : ℝ), f.to_fun (δ*y)) x = (1/δ) * real.fourier_integral f.to_fun (x/δ) :=
begin
  rw fourier_integral_eq_integral_exp_smul,
  rw fourier_integral_eq_integral_exp_smul,
...
lemma cexp_sqrt_delta_eq_cexp_abs {x δ:ℝ} (h: 0 < δ): cexp (-(real.pi:ℂ) * ↑(x / δ ^ ((1 / 2):ℝ)) ^ 2) = cexp (-(↑real.pi / ↑δ) * ↑|x| ^ 2):=
begin
  norm_cast,
  rw div_pow,
  rw div_eq_mul_one_div,
...
lemma fourier_integral_norm_sqrt_delta {x δ:ℝ} (h: 0 < δ): real.fourier_integral  (λ x : ℝ, complex.exp (-((real.pi*δ):ℂ )* ‖x‖^2)) x = real.fourier_integral  (λ x : ℝ, (λy:ℝ,complex.exp (-((real.pi):ℂ )* ‖y‖^2)) ((δ^((1/2):ℝ ))*x)) x:=
begin
  congr,
  dsimp,
  norm_cast,
...
lemma gaussian_pi_eq_gaussian_complex_pi {x δ:ℝ} (h: 0 < δ): 𝓕 gaussian_pi.to_fun (x / δ ^ ((1 / 2):ℝ)) = 𝓕 gaussian_complex_pi.to_fun (x / δ ^ ((1 / 2):ℝ)):=
begin
  rw fourier_integral_eq_integral_exp_smul,
  rw fourier_integral_eq_integral_exp_smul,
  congr,
...
lemma good_kernel_fourier_transform(x:ℝ) {δ:ℝ} (hδ: 0<δ) : real.fourier_integral  (λ x : ℝ, complex.exp (-((real.pi*δ):ℂ )* ‖x‖^2)) x  = (λ x : ℝ, (sqrt (1 / δ):ℂ)  * complex.exp ((-((real.pi/δ):ℂ )* ‖x‖^2))) x:=
begin
  have h₁: 0< ((real.pi*δ):ℂ ).re, by {norm_cast,rw zero_lt_mul_right hδ,exact pi_pos,},
  have h₃: real.fourier_integral  (λ x : ℝ, complex.exp (-((real.pi*δ):ℂ )* ‖x‖^2)) x = real.fourier_integral  (λ x : ℝ, (λy:ℝ,complex.exp (-((real.pi):ℂ )* ‖y‖^2)) ((δ^((1/2):ℝ ))*x)) x, by {refine fourier_integral_norm_sqrt_delta hδ,},
...
theorem integral_good_kernel_eq_one {δ:ℝ } (hδ: 0<δ): ∫ x:ℝ,  (1 / (δ:ℂ )^ ((1 /2):ℤ))*cexp (- (real.pi/δ)* x^2) = 1 :=
begin
...
-- # MEDIUM SORRY
lemma iterated_fderiv_add {f: schwartz_map ℝ ℝ} {x h:ℝ} {n:ℕ}: iterated_fderiv ℝ n f.to_fun (h + x) =  iterated_fderiv ℝ n (λ (y : ℝ), f.to_fun (h + y)) x:=
begin
  induction n with n hn,
  ...
  rw iterated_fderiv_succ_eq_comp_right,
  simp only [linear_isometry_equiv.map_eq_iff],
  sorry,
  /-rw ← hn,
  congr,
  have h₁:iterated_fderiv ℝ n (λ (y : ℝ), f.to_fun (h + y)) = iterated_fderiv ℝ n (λ (y : ℝ), f.to_fun (2*h + y)), sorry,
  rw h₁,
  rw ← hn,
  --rw [iterated_fderiv_eq_equiv_comp, iterated_fderiv_eq_equiv_comp,iterated_deriv_succ',iterated_deriv_succ'],
  --rw [iterated_fderiv_eq_equiv_comp, iterated_fderiv_eq_equiv_comp] at hn,
  iterated_deriv_eq_iterate, linear_isometry_equiv.norm_map,← iterated_deriv_eq_iterate, iterated_deriv_succ'],
  rw [iterated_fderiv_eq_equiv_comp, iterated_deriv_eq_iterate, linear_isometry_equiv.norm_map,← iterated_deriv_eq_iterate, iterated_deriv_succ'],
  simp only [norm_eq_abs],
  have h₁: deriv (λ (y : ℝ), f.to_fun (h + y)) = (λ (y : ℝ), deriv f.to_fun (h + y)), 
    simp only,
  rw h₁,
  sorry,-/
end

lemma iterated_fderiv_add2 {f: schwartz_map ℝ ℂ} {x h:ℝ} {n:ℕ}: ‖iterated_fderiv ℝ n f.to_fun (h + x)‖ =  ‖iterated_fderiv ℝ n (λ (y : ℝ), f.to_fun (h + y)) x‖:=
begin
  induction n with n hn,
  simp only [norm_iterated_fderiv_zero],
  rw [iterated_fderiv_eq_equiv_comp, iterated_deriv_eq_iterate, linear_isometry_equiv.norm_map,← iterated_deriv_eq_iterate, iterated_deriv_succ'],
  rw [iterated_fderiv_eq_equiv_comp, iterated_deriv_eq_iterate, linear_isometry_equiv.norm_map,← iterated_deriv_eq_iterate, iterated_deriv_succ'],
  have h₁: deriv (λ (y : ℝ), f.to_fun (h + y)) = (λ (y : ℝ), deriv f.to_fun (h + y)), sorry,
  rw h₁,
  sorry,
end

lemma iterated_fderiv_sub {f: schwartz_map ℝ ℂ} {x h:ℝ} {n:ℕ}: ‖iterated_fderiv ℝ n f.to_fun (h - x)‖ =  ‖iterated_fderiv ℝ n (λ (y : ℝ), f.to_fun (h - y)) x‖:=
begin
  sorry,
end


lemma schwartz_add_bound_sum {f: schwartz_map ℝ ℂ} {x h:ℝ} {n k: ℕ}: ‖x‖ ^ k * ‖iterated_fderiv ℝ n f.to_fun (h + x)‖ ≤ (‖x + h‖ + ‖h‖)^k * ‖iterated_fderiv ℝ n f.to_fun (h + x)‖:=
begin
...
lemma schwartz_sub_bound_sum {f: schwartz_map ℝ ℂ} {x h:ℝ} {n k: ℕ}: ‖x‖ ^ k * ‖iterated_fderiv ℝ n f.to_fun (h - x)‖ ≤ (‖x - h‖ + ‖h‖)^k * ‖iterated_fderiv ℝ n f.to_fun (h - x)‖:=
begin
  have h₆: ‖x‖^k ≤ (‖x - h‖ + ‖h‖)^k,
...
lemma schwartz_sum_decay (f: schwartz_map ℝ  ℂ ) :∀ (k n : ℕ), ∃ (C : ℝ), ∀ (x : ℝ), ∑ (m : ℕ) in range (k + 1), ‖x‖ ^ m * ‖iterated_fderiv ℝ n f.to_fun x‖ ≤ C:=
begin
  intro k,
...
lemma zero_le_sum_norm_mul_norm {f: schwartz_map ℝ ℂ} {a b: ℝ } {k n:ℕ}: 0 ≤ ∑ (m : ℕ) in range (k + 1), ‖a‖ ^ m * ‖iterated_fderiv ℝ n f.to_fun (b)‖:=
begin 
  induction k with k hk,
...
lemma sum_le_sum {f g: ℕ → ℝ} {k:ℕ} (h: ∀ i:ℕ, i≤ k →  f i ≤ g i): ∑ (m : ℕ) in range (k + 1), f m ≤ ∑ (m : ℕ) in range (k + 1), g m :=
begin
  induction k with k hk,
...
lemma sum_pow_mul_schwartz_le_const_mul_sum_schwartz {f: schwartz_map ℝ ℂ} {k n:ℕ} {x h:ℝ}: ∑ (m : ℕ) in range (k + 1), ‖x + h‖ ^ m * ‖h‖ ^ (k - m) * ↑(k.choose m) * ‖iterated_fderiv ℝ n f.to_fun (h + x)‖ ≤ ∑ (m : ℕ) in range (k + 1),  (2^k * (1+‖h‖) ^ (k)) * ‖x + h‖ ^ m  * ‖iterated_fderiv ℝ n f.to_fun (h + x)‖:=
begin
  rw [← sum_mul,←sum_mul],
  refine mul_le_mul _ rfl.ge (norm_nonneg _) _,
...
lemma sum_pow_mul_schwartz_le_const_mul_sum_schwartz_sub {f: schwartz_map ℝ ℂ} {x h:ℝ} {k n:ℕ}: ∑ (m : ℕ) in range (k + 1), ‖x - h‖ ^ m * ‖h‖ ^ (k - m) * ↑(k.choose m) * ‖iterated_fderiv ℝ n f.to_fun (h - x)‖ ≤ ∑ (m : ℕ) in range (k + 1),  (2^k * (1+‖h‖) ^ (k)) * ‖x - h‖ ^ m  * ‖iterated_fderiv ℝ n f.to_fun (h - x)‖:=
begin
  rw [← sum_mul,←sum_mul],
  refine mul_le_mul _ rfl.ge (norm_nonneg _) _,
  {simp_rw [mul_comm _ (‖x - h‖ ^ _), mul_comm _ ((1 + ‖h‖)^_), ←mul_assoc],
...
begin
  ...
  exact C,
  have h₁: ∀a:ℝ, cexp (-(↑real.pi / ↑δ) * ↑‖a‖ ^ 2) = (λy:ℝ, cexp (-(↑real.pi / ↑δ) * ↑‖y‖ ^ 2)) a, by {intro a, simp,},
  simp_rw h₁ _,
...
lemma delta_mu_are_different1 (g: schwartz_map ℝ  ℂ ) (δ C:ℝ): ∀ x:ℝ,tendsto (λ (μ  : ℝ), (∫ (t : ℝ) in Iio (-μ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)) + ∫ (t : ℝ) in Ioi μ , 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)) (nhds_within 0 (Ioi 0)) (nhds ( (1 / ↑δ ^ (1 / 2))* ((↑real.pi / (↑real.pi / ↑δ)) ^ ((1:ℂ) / 2) / 2) + (- ((1 / ↑δ ^ (1 / 2))* ((↑real.pi / (↑real.pi / ↑δ)) ^ ((1:ℂ) / 2) / 2))))):=
begin
  intro x,
...
--- here C is something like C1*(↑real.pi / (↑real.pi / ↑δ)) ^ (1 / 2) / 2, where C1 is a schwartz bound for g(x-y)-g(x)
lemma delta_mu_are_different2 (g: schwartz_map ℝ  ℂ ) (δ C:ℝ): ∀ x:ℝ,tendsto (λ (μ  : ℝ), (∫ (t : ℝ) in Iio (-μ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)) + ∫ (t : ℝ) in Ioi μ , 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)) (nhds_within 0 (Ioi 0)) (nhds (C + (-C))):=
begin
  /-intro x,
  refine filter.tendsto.add _ _,
  rw metric.tendsto_nhds_within_nhds,
  ...
  intro h, specialize h₁ h,
  rw dist_eq_norm, rw dist_eq_norm at h₁,
  have h₂: (C:ℂ) =  (∫ (t : ℝ) in Iio (-0), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)) - (∫ (t : ℝ) in Iio (-0), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)) + (C:ℂ), sorry,
  rw h₂,
  rw sub_add_eq_sub_sub,
  rw ← sub_add,
  rw add_sub_assoc,
  ...
  dedup,-/
  --simp_rw ← add_zero,
  --have h₂:= norm_sub_le (∫ (y : ℝ) in Iio (-μ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x))  (∫ (y : ℝ) in Iio (-0), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x)),
  --refine lt_trans _ h₁, 
  --refine lt_of_le_of_lt (norm_sub_le _ _) _,
  --refine lt_of_lt_of_le _ (norm_sub_le _ _),
end
...
lemma delta_is_mixed (g: schwartz_map ℝ  ℂ ) (C x:ℝ): tendsto (λ (δ : ℝ), (∫ (y : ℝ) in Iio (-δ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖y‖ ^ 2) * (g.to_fun (x -y) - g.to_fun x)) + ∫ (y : ℝ) in Ioi δ, 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x)) (nhds_within 0 (Ioi 0)) (nhds (C + (-C))):=
begin
  sorry,
end
...
begin 
  ...
  --let μ : ℝ,
 -- set μ := ℝ  with hμ,
  specialize h₁ δ, -- i want to define a positive μ and then use it. Now I use δ instead
  specialize h₁ hδ,
...
#check implies (0<1) (1<2)

#check ℝ  

/-

lemma differentiable_iterated_deriv_fourier_transform {f: schwartz_map ℝ ℂ} {n:ℕ}: 
differentiable ℝ (λ w:ℝ, (∫ (v : ℝ), iterated_deriv n (λw:ℝ,cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) w)) :=
begin
  unfold differentiable,
  intro x,
  refine has_deriv_at.differentiable_at _,
end




lemma deriv_cexp_bound2 (h:ℝ): 
∃ (C : ℝ), ∀ (x : ℝ), ‖deriv (λ (y : ℝ), cexp (↑((-2) * real.pi * y * h) * I)) x‖ ≤ C:=
begin
  use (2 * |real.pi| * |h|),
  intro x,
...
lemma integral_moderate_decrease_bounded2 {C:ℝ} (l:filter ℝ  ):∃ I:ℝ,  ∀ᶠ (i : ℝ  ) in l, ∫ (x : ℝ) in (λy:ℝ ,-‖y‖) i..(λy:ℝ ,‖y‖) i, ‖(λ (x : ℝ), (C * ‖x‖) / (1 + ‖x‖ ^ 2)) x‖ ≤ I:=
begin
  set a:= (λy:ℝ ,-‖y‖) with ha,
  set b:= (λy:ℝ ,‖y‖) with hb,
  use (((real.pi/2+1)+(real.pi/2+1))*‖C‖),
  refine filter.eventually_of_forall _,
  intro i,
  have h₁: ∫ (x : ℝ) in a i..b i, ‖(λ (x : ℝ), (C*‖x‖) / (1 + ‖x‖ ^ 2)) x‖ = (∫ (x : ℝ) in a i..b i, ‖x‖ / (1 + x ^ 2))*‖C‖, 
  {sorry,},
  rw [h₁, integral_one_div_one_add_sq, sub_eq_add_neg],
  refine mul_le_mul _ rfl.ge (norm_nonneg _) _,
  refine add_le_add _ _,
...
lemma hello {x C:ℝ}:  (C)/ (1+ (‖x‖*(1+‖x‖^2))) ≤ (C)/ (‖x‖ * (1+‖x‖^2)):=
begin
  rw div_eq_mul_one_div,
  nth_rewrite 1 div_eq_mul_one_div,
  refine mul_le_mul rfl.ge _ _ _,
...
lemma moderate_decrease_pow_version2 (f: schwartz_map ℝ ℂ):
∀ k:ℕ, ∃ C:ℝ, ∀ x:ℝ, ‖x‖^k * ‖f.to_fun x‖ ≤  (C)/ (‖x‖ * (1+‖x‖^2)):=
begin
  have h₁:= f.decay',
  have h₂:= f.decay',
  intro k,
  specialize h₁ (k+1),  specialize h₂ (k+3),
  specialize h₁ 0,  specialize h₂ 0,
  obtain ⟨C₁,h₁⟩:=h₁,
  obtain ⟨C₂,h₂⟩:=h₂,
  use (C₁ + C₂),
  intro x,
  have h₃: (C₁ + C₂)/ (1+ (‖x‖*(1+‖x‖^2))) ≤ (C₁ + C₂)/ (‖x‖ * (1+‖x‖^2)), sorry,
  refine le_trans _ h₃,
  rw [le_div_iff _, mul_comm, mul_add, add_mul, mul_one, ←mul_assoc],-- ← pow_add _ _, add_comm 2 k],
  specialize h₁ x,  specialize h₂ x,
  simp only [pow_zero, norm_iterated_fderiv_zero, one_mul] at h₁, simp only [norm_iterated_fderiv_zero] at h₂,
  refine add_le_add _ _,
  convert h₁, 
...
lemma helloh {a:ℝ }: ‖a‖ / (1 + ‖a‖ ^ 2) ≤ 1 / (1 + ‖a‖ ^ 2) :=
begin
end

lemma deriv_cexp_mul_schwartz_moderate_decrease2 (f:schwartz_map ℝ ℂ) : 
∀ x:ℝ,∃ (C:ℝ), ∀ᵐ (a : ℝ), ∀ (x_1 : ℝ), x_1 ∈ metric.ball x (‖C‖+1) → ‖deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x_1‖ ≤ (C*‖a‖) / (1 + ‖a‖ ^ 2):=
begin
  have h₁:=moderate_decrease_pow f,
  specialize h₁ 0, 
  intro x, 
  ...
  refine eventually_of_forall _,  
  intro y,  intros w hw,
  have h₂:= deriv_cexp_bound25 y,
  specialize h₁ y, 
  specialize h₂ w, 
  rw [pow_zero,one_mul] at h₁,
  simp only [deriv_mul_const_field', norm_mul, pow_bit0_abs],
  rw mul_div_assoc,
  have h₃: C₁ * 2 * |real.pi| * (‖y‖ / (1 + ‖y‖ ^ 2)) =  (2 * |real.pi|* ‖y‖)* (C₁/ (1 + ‖y‖ ^ 2)), by ring_nf,
  rw h₃,
  convert mul_le_mul h₂ h₁ (norm_nonneg _) _,
  ext1 b,
  repeat {rw ←neg_mul,},
  rw [mul_comm _ I, mul_comm _ I],
  congr,
...
lemma deriv_cexp_mul_schwartz_moderate_decrease3 (f:schwartz_map ℝ ℂ) : 
∀ x:ℝ,∃ (C:ℝ), ∀ᵐ (a : ℝ), ∀ (x_1 : ℝ), x_1 ∈ metric.ball x (‖C‖+1) → ‖deriv (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑a * ↑w * I)) * f.to_fun a) x_1‖ ≤ (C) / (1 + ‖a‖ ^ 2) +1 :=
begin
  have h₁:=deriv_cexp_mul_schwartz_moderate_decrease2 f,
  intro x,
  specialize h₁ x, 
  obtain ⟨C,h₁⟩:=h₁,
  use C,
...
lemma integrable_norm_div_one_add_sq (C:ℝ): 
measure_theory.integrable (λ (v : ℝ), (λ (x : ℝ), (C* ‖x‖ / (1 + ‖x‖ ^ 2))) v) measure_theory.measure_space.volume:=
begin
  have h:= integral_moderate_decrease_bounded2 at_top, 
  obtain ⟨I,h⟩:=h,
  refine measure_theory.integrable_of_interval_integral_norm_bounded I _ _ _ h,
  {intro i,
  refine continuous.integrable_on_Ioc _,
...
lemma integrable_restrict {δ C:ℝ} (h: measure_theory.integrable (λ (x : ℝ), C / (1 + ‖x‖ ^ 2)) (measure_theory.measure_space.volume)) :measure_theory.integrable (λ (x : ℝ), C / (1 + ‖x‖ ^ 2)) (measure_theory.measure_space.volume.restrict (Ioi δ)):=
begin
  sorry,
end
...
lemma Icc_tendsto_zero1 (g: schwartz_map ℝ  ℂ ): 
∀ x:ℝ,tendsto  (λ μ:ℝ, ∫ (y : ℝ) in Icc (-μ) (μ), 1 / ↑μ ^ (1 / 2) * cexp (-(↑real.pi / ↑μ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x)) (nhds_within 0 (set.Ioi 0)) (nhds (0)):=
begin
  sorry,
end

lemma bound (g: schwartz_map ℝ  ℂ ) (δ y:ℝ):  ∃C:ℝ, ∀x:ℝ,  ‖cexp (-(↑real.pi / ↑δ) * ↑‖x‖ ^ 2)‖ * ‖g.to_fun (y - x) - g.to_fun y‖ ≤ C / (1 + ‖x‖ ^ 2):=
begin
  have h₁:=  moderate_decrease (schwartz_sub g x),
  have h₂:= g.decay',
  specialize h₂ 0, specialize h₂ 0,
  obtain ⟨C₁,h₁⟩:=h₁, obtain ⟨C₂,h₂⟩:=h₂, 
  simp only [pow_zero, norm_iterated_fderiv_zero, one_mul] at h₂,
  specialize h₁ y, specialize h₂ y,
  use (C₁ + ‖g.to_fun y‖),
  rw ←one_mul (C₁ + ‖g.to_fun y‖),
  rw mul_div_assoc,
...
lemma good_kernel_property_positive1 (g: schwartz_map ℝ  ℂ ): 
∀ x:ℝ,tendsto  (λ μ:ℝ, ∫ (y : ℝ) in Ioi (μ), 1 / ↑μ ^ (1 / 2) * cexp (-(↑real.pi / ↑μ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x)) (nhds_within 0 (set.Ioi 0)) (nhds (0)):=
begin
  intro x,
  rw metric.tendsto_nhds_within_nhds,
  intros ε hε,
  ...
  have h₁:=bound g δ x,
  obtain ⟨C,h₁⟩:=h₁,
  refine lt_of_le_of_lt (measure_theory.norm_integral_le_of_norm_le  _ _) _,  
  refine (λ (x : ℝ), ((‖1 / (δ:ℂ) ^ (1 / 2)‖* C) / (1 + ‖x‖ ^ 2))),
  refine integrable_restrict _,
  refine integrable_moderate_decrease _,
  refine eventually_of_forall _,
  intro y,
  ...
  rw mul_assoc, rw mul_assoc,
  refine mul_le_mul rfl.ge _ _ (norm_nonneg _),
  refine le_trans (h₁ y) _,
  rw div_eq_mul_one_div,
  positivity,
  nth_rewrite 0 ← one_mul (1),
end

lemma n_top_integral_real (f: ℝ→ ℝ) (hf: measure_theory.integrable (λ (v : ℝ), f v) measure_theory.measure_space.volume):
tendsto (λ (n : ℝ), (λ (m : ℝ), ∫ (v : ℝ) in m..n, f v)) at_top (nhds (λ (m : ℝ),∫ (v : ℝ) in Ioi (m), f v)) :=
begin
  rw metric.tendsto_at_top,
  have h₁:= measure_theory.interval_integral_tendsto_integral_Ioi _ _ _,
  {simp only [tendsto_neg_at_bot_iff], exact rfl.ge,},
end

lemma good_kernel_property_positive1new (g: schwartz_map ℝ  ℂ ): 
∀ x:ℝ,tendsto  (λ μ:ℝ, ∫ (y : ℝ) in Ioi (μ), 1 / ↑μ ^ (1 / 2) * cexp (-(↑real.pi / ↑μ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x)) (nhds_within 0 (set.Ioi 0)) (nhds (0)):=
begin
  intro x,
  have h₁:= measure_theory.interval_integral_tendsto_integral_Ioi _ _ _,
  have h₃: tendsto (λn:ℝ, λ (μ : ℝ), ∫ (y : ℝ) in μ..n, 1 / ↑μ ^ (1 / 2) * cexp (-(↑real.pi / ↑μ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x)) at_top (nhds(λ (μ : ℝ), ∫ (y : ℝ) in Ioi μ, 1 / ↑μ ^ (1 / 2) * cexp (-(↑real.pi / ↑μ) * ↑‖y‖ ^ 2) * (g.to_fun (x - y) - g.to_fun x))),
    refine n_top_integral_real _ _,
    refine integrable_moderate_decrease _,
  refine tendsto_nhds_unique_le h₃ _,
  rw metric.tendsto_nhds_within_nhds,
  intros ε hε,
...
begin
  ...
  rw sub_zero,
  refine lt_of_le_of_lt (measure_theory.norm_integral_le_of_norm_le  _ _) _,  
  refine (λ (x : ℝ), (‖1 / (δ:ℂ) ^ (1 / 2)‖ / (1 + ‖x‖ ^ 2))),
  sorry, --refine integrable_restrict _,
  refine eventually_of_forall _,
  intro x,
  rw [norm_mul,norm_mul],
  nth_rewrite 2 div_eq_mul_one_div,
...
begin 
  intro x,
  refine tendsto_nhds_within_congr _ _,
  refine λδ:ℝ,∫ (t : ℝ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x),
  intros δ hδ,
  rw set.mem_Ioi at hδ,
  have h₁:= integral_le_3_set_integrals2 g hδ,
  specialize h₁ δ,
  specialize h₁ hδ,
  rw good_kernel_rw22 hδ,
  refine fourier_convolution_good_kernel_conversion_3part g x,
end-/


/-lemma fourier_convolution_good_kernel_conversion_proper2_p  (g: schwartz_map ℝ  ℂ ):
∀ x:ℝ,tendsto (λ δ:ℝ , ∫ (t: ℝ), g.to_fun (x-t) * (λ x : ℝ ,1 / (δ:ℂ ) ^ (1 / 2) *complex.exp (- (real.pi/δ)*‖x‖^2))  t ) (nhds_within 0 (set.Ioi 0)) (nhds (g.to_fun x)):= 
begin 
  intro x,
  have h₁: (λ δ:ℝ , (∫ (t : ℝ), g.to_fun (x - t) * (1 / ↑δ ^ (1 / 2) * cexp (- (real.pi/δ)*‖t‖^2))) - g.to_fun x) = (λ δ:ℝ ,∫ (t : ℝ), 1 / ↑δ ^ (1 / 2) * cexp (- (real.pi/δ)*‖t‖^2) * (g.to_fun (x - t) - g.to_fun x)),
    ext1 δ, --congr,
    nth_rewrite 0 ← one_mul (g.to_fun x),
  nth_rewrite 0 ←integral_good_kernel_eq_one hδ, 
  repeat {rw  ← smul_eq_mul,},
  ...
  rw this,
  rw set.mem_Ioi at hδ,
  refine integrable_fun1 g hδ, 
  refine integrable_fun2 g hδ,
  rw h₁, 
  have h₂:= good_kernel_property_3 g δ, 
  specialize h₂ x,
  rw metric.tendsto_nhds_within_nhds at h₂,
  specialize h₂ ε, specialize h₂ hε,
  refine lt_of_le_of_lt (yes2 g δ hδ) _, -- need to find a way to transform (-inf,inf) into  (-inf,-n) + (-n,n) + (n,inf)
  have h₂: ‖∫ (t : ℝ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)‖ ≤∫ (t : ℝ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * ‖(g.to_fun (x - t) - g.to_fun x)‖ , by sorry,
  have h₃: ∫ (t : ℝ) in -η.. η, 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * ‖(g.to_fun (x - t) - g.to_fun x)‖ ,
end-/
...
begin 
  ...
  intro h,
  rw dist_eq_norm, 
  have h₁:  ‖(∫ (t : ℝ), g.to_fun (x - t) * (1 / ↑δ ^ (1 / 2) * cexp (- (real.pi/δ)*‖t‖^2))) - g.to_fun x‖=‖∫ (t : ℝ), 1 / ↑δ ^ (1 / 2) * cexp (- (real.pi/δ)*‖t‖^2) * (g.to_fun (x - t) - g.to_fun x)‖,
    congr,
    nth_rewrite 0 ← one_mul (g.to_fun x),
  nth_rewrite 0 ←integral_good_kernel_eq_one hδ, 
  repeat {rw  ← smul_eq_mul,},
  ...
  refine integrable_fun2 g hδ,
  rw h₁, 
  have h₂:= good_kernel_property_3 g, 
  specialize h₂ x,
  rw metric.tendsto_nhds_within_nhds at h₂,
  specialize h₂ ε, specialize h₂ hε,
  refine lt_of_le_of_lt (yes2 g δ hδ) _, -- need to find a way to transform (-inf,inf) into  (-inf,-n) + (-n,n) + (n,inf)
...
/-lemma yes4 (g: schwartz_map ℝ  ℂ ) {x δ:ℝ}: 
∀ μ:ℝ, 0< μ →  ∫ (t : ℝ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x) = 
(∫ (x_1 : ℝ) in Iio (-μ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖x_1‖ ^ 2) * (g.to_fun (x - x_1) - g.to_fun x)) + (∫ (x_1 : ℝ) in Icc (-μ) μ, 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖x_1‖ ^ 2) * (g.to_fun (x - x_1) - g.to_fun x)) + ∫ (x_1 : ℝ) in Ioi μ, 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖x_1‖ ^ 2) * (g.to_fun (x - x_1) - g.to_fun x) :=
begin
  have h₁: ∀ μ:ℝ, measurable_set (set.Iio (-μ)), by {intro μ,refine measurable_set_Iio,},
  ...
  rw add_assoc,
  refine measure_theory.integrable.integrable_on _,
  refine integrable_good_kernel_expression _,
  refine measure_theory.integrable.integrable_on _,
  refine integrable_good_kernel_expression _,
  refine integrable_good_kernel_expression _,
end-/

/-lemma yes3 (g: schwartz_map ℝ  ℂ ) (x δ:ℝ): 
 ‖∫ (t : ℝ), 1 / ↑δ ^ (1 / 2) * cexp (-(↑real.pi / ↑δ) * ↑‖t‖ ^ 2) * (g.to_fun (x - t) - g.to_fun x)‖ ≤  1:=
begin
  have h₁: ∀ μ:ℝ, measurable_set (set.Iio (-μ)), by {intro μ,refine measurable_set_Iio,},
  intros μ hμ, 
  have h₂: ∀ μ:ℝ, measurable_set (set.Ioi μ), by {intro μ, refine measurable_set_Ioi,},
  have h₃: (Iio (-μ))ᶜ = (Ici (-μ)), by {simp only [compl_Iio],},
  have h₄: Ici (-μ) = Icc (-μ) (μ) ∪ Ioi (μ), by {rw set.Icc_union_Ioi_eq_Ici, refine neg_le_self hμ,},
  rw [← measure_theory.integral_add_compl (h₁ μ) _,h₃,h₄,measure_theory.integral_union (Icc_disjoint_Iio hμ) (h₂ μ ) _ _],
  refine le_trans (norm_add_le _ _) _,
  rw add_assoc,
  refine add_le_add rfl.ge _,
  refine le_trans (norm_add_le _ _) _,
  refine add_le_add rfl.ge rfl.ge,
  refine measure_theory.integrable.integrable_on _,
  refine integrable_good_kernel_expression _,
  refine measure_theory.integrable.integrable_on _,
  refine integrable_good_kernel_expression _,
...
/-lemma good_kernel_property_3 (g: schwartz_map ℝ  ℂ ) {δ:ℝ}: 
∀ x:ℝ,tendsto (λ δ:ℝ , ∫ (t: ℝ), g.to_fun (x-t) * (λ x : ℝ ,1 / (δ:ℂ ) ^ (1 / 2) *complex.exp (- (real.pi/δ)*‖x‖^2))  t ) (nhds_within 0 (set.Ioi 0)) (nhds (g.to_fun x)):=
begin
  sorry,
end-/

/-lemma tendsto_moderate_decrease {C:ℝ}: 
∃ n:ℝ ,  ∫ (x : ℝ), ‖C‖ / (1 + ‖x‖ ^ 2) =  (λn:ℝ, ∫ (x : ℝ) in -n .. n, ‖C‖ / (1 + ‖x‖ ^ 2)) n:=
begin
  have h₁:= n_top_integral_real (λ(x : ℝ), ‖C‖ / (1 + ‖x‖ ^ 2)) _,
  rw metric.tendsto_at_top at h₁,
  specialize h₁ 1,
  ...
  specialize h₁ (N+1),
  specialize h₁ _,
  simp only [ge_iff_le, le_add_iff_nonneg_right, zero_le_one],
...
begin
  ...
  simp_rw rewrite3,
  refine integrable_moderate_decrease _,
  have h₃: ∃ n:ℝ ,  ∫ (x : ℝ), ‖C‖ / (1 + ‖x‖ ^ 2) =  (λn:ℝ, ∫ (x : ℝ) in -n .. n, ‖C‖ / (1 + ‖x‖ ^ 2)) n,
    refine tendsto_moderate_decrease,
  obtain ⟨n,h₃⟩:=h₃,
  rw h₃,
  dsimp only,
  have h₄: ∫ (x : ℝ) in -(n:ℝ )..(n:ℝ ), ‖C‖ / (1 + ‖x‖ ^ 2) = (∫ (x : ℝ) in -(n:ℝ)..(n:ℝ ), (1 + x ^ 2)⁻¹) * (‖C‖),
    {rw [←smul_eq_mul,←  interval_integral.integral_smul_const], congr, ext1,
    rw [div_eq_mul_one_div, mul_comm, ← smul_eq_mul,inv_eq_one_div],
    simp only [norm_eq_abs, pow_bit0_abs],},
  rw [h₄, integral_inv_one_add_sq, sub_eq_add_neg, smul_eq_mul,mul_comm (‖C‖ + ‖C‖) _, add_mul,mul_add],
  refine add_le_add _ _,
  refine mul_le_mul _ rfl.ge (norm_nonneg _) (le_of_lt zero_lt_pi_div_two_pos_plus_one),
  refine le_trans (le_of_lt (arctan_lt_pi_div_two (n))) _,
  simp only [le_add_iff_nonneg_right, zero_le_one],
  refine mul_le_mul _ rfl.ge (norm_nonneg _) (le_of_lt zero_lt_pi_div_two_pos_plus_one),
  {rw neg_le,  refine le_trans _ (le_of_lt (real.neg_pi_div_two_lt_arctan (-n))),
  refine neg_le_neg _,  simp only [le_add_iff_nonneg_right, zero_le_one],},
end-/

-- # Related to sorry
/-lemma tendsto_moderate_decrease {C:ℝ} : 
tendsto (λn:ℝ, ∫ (x : ℝ) in -n .. n, ‖C‖  / (1 + ‖x‖ ^ 2)) at_top (nhds ((λ (n : ℝ), ∫ (x : ℝ) in -n..n, ‖C‖  / (1 + ‖x‖ ^ 2)) ↑n)):=
begin
  have h₁:= n_top_integral_real (λx:ℝ,‖C‖  / (1 + ‖x‖ ^ 2)) _,
  rw metric.tendsto_at_top at h₁, 
  specialize h₁ _, specialize h₁ _,
  obtain ⟨n,h₁⟩:=h₁,
  use n,
  intros m hm,
  rw dist_eq_norm,
...
lemma tendsto_moderate_decrease2 {C:ℝ} {n:ℕ}: 
∃ n:ℕ, tendsto (λn:ℝ, ∫ (x : ℝ) in -n .. n, ‖C‖  / (1 + ‖x‖ ^ 2)) at_top (nhds ((λ (n : ℝ), ∫ (x : ℝ) in -n..n, ‖C‖  / (1 + ‖x‖ ^ 2)) ↑n)):=
begin
  use 1,
  suggest,
  rw metric.tendsto_at_top, 
...
lemma cexp_mul_schwartz_bounded(f:schwartz_map ℝ ℂ) {n:ℕ} : 
∃ (C:ℝ), ∀ x:ℝ, ‖∫ (v : ℝ), cexp (↑((-2) * real.pi * v * x) * I) • (f.to_fun v)‖ ≤ C :=
begin
  have h₂:= cexp_mul_schwartz_moderate_decrease f,
  obtain ⟨C,h₂⟩:=h₂,
  ...
  simp_rw rewrite3,
  refine integrable_moderate_decrease _,
  have h₃:  ∫ (x : ℝ), ‖C‖ / (1 + ‖x‖ ^ 2) =  (λn:ℝ, ∫ (x : ℝ) in -n .. n, ‖C‖ / (1 + ‖x‖ ^ 2)) n,
    --have h₄: tendsto (λn:ℝ, ∫ (x : ℝ) in -n .. n, ‖C‖  / (1 + ‖x‖ ^ 2)) at_top (nhds ((λ (n : ℝ), ∫ (x : ℝ) in -n..n, ‖C‖  / (1 + ‖x‖ ^ 2)) ↑n)), sorry,
    refine tendsto_nhds_unique _ (tendsto_moderate_decrease),
    refine n_top_integral_real (λ x:ℝ, ‖C‖ / (1 + ‖x‖ ^ 2)) (integrable_moderate_decrease _),
  rw h₃,
  dsimp only,
  have h₄: ∫ (x : ℝ) in -(n:ℝ )..(n:ℝ ), ‖C‖ / (1 + ‖x‖ ^ 2) = (∫ (x : ℝ) in -(n:ℝ)..(n:ℝ ), (1 + x ^ 2)⁻¹) * (‖C‖),
    {rw [←smul_eq_mul,←  interval_integral.integral_smul_const], congr, ext1,
...
/-
lemma hhh (a: ℝ→ℝ) (i:ℕ ): -arctan (a i) ≤ real.pi / 2 + 1:=
begin
  rw neg_le,
  have h:= real.neg_pi_div_two_lt_arctan (a i),
...
/-
lemma moderate_decrease (x:ℝ) (C₁ C₂:ℝ ) (h₁: 0≤C₁) (h₂: 0≤C₂):
max C₁ (C₂/‖x‖^2) ≤ (max C₁ C₂)* max 1 (1/‖x‖^2):=
begin
  simp only [norm_eq_abs, pow_bit0_abs, max_le_iff],
  split,nth_rewrite 0 ←mul_one C₁, refine mul_le_mul _ _ zero_le_one _, rw le_max_iff,  left, exact rfl.ge,
  rw le_max_iff,  left, exact rfl.ge,rw le_max_iff,  left, exact h₁,
  rw div_eq_mul_one_div, refine mul_le_mul _ _ _ _, rw le_max_iff,  right,exact rfl.ge,rw le_max_iff,  right,exact rfl.ge,
  simp only [one_div, inv_nonneg], positivity, rw le_max_iff,  left, exact h₁,
end

lemma moderate_decrease2 (x:ℝ) (C₃:ℝ ) (h₁: 0≤C₃):
C₃* max 1 (1/‖x‖^2)≤C₃ * 2/ (1 + ‖x‖^2 ) :=
begin
  nth_rewrite 1 div_eq_mul_one_div, 
  rw mul_assoc,
  rw ← div_eq_mul_one_div,
  refine mul_le_mul rfl.ge _ _ _, --sorry, 
  cases le_or_lt (‖x‖) 1,
  rw max_le_iff,
  split,
  nth_rewrite 0 ← one_div_one,
  --simp only [norm_eq_abs, pow_bit0_abs, one_div],
  refine div_le_div _ _ _ _,
  rw one_div_one, exact zero_le_one, rw one_div_one, positivity,
end
-/

/-lemma n_top_integral {f: ℝ → ℂ } : tendsto (λ (n : ℝ), ∫ (v : ℝ) in -n..n, f v) at_top (nhds (∫ (v : ℝ), f v)) :=
begin
  refine measure_theory.interval_integral_tendsto_integral _ _ rfl.ge,
  refine measure_theory.integrable_of_interval_integral_norm_bounded _ _ _ _ _,
  sorry, -- integrable
  simp only [tendsto_neg_at_bot_iff], exact rfl.ge,
end-/
...
/-lemma prev  {f : C(ℝ , C(ℝ , ℂ))} (h:continuous f): measure_theory.ae_strongly_measurable (function.uncurry f) (measure_theory.measure_space.volume.prod measure_theory.measure_space.volume):=
begin
  refine continuous.ae_strongly_measurable _,
  convert continuous_map.continuous_uncurry,
  --convert continuous_map.continuous_uncurry_of_continuous _,
  --sorry,
  --squeeze_simp,
end -/
...
/-lemma multiplication_formula_step2 (f : ℝ → ℝ → ℂ) 
(ha:measure_theory.integrable (function.uncurry f) (measure_theory.measure_space.volume.prod measure_theory.measure_space.volume)):
 ∫ (x : ℝ ), ∫ (y : ℝ ), f x y = ∫ (y : ℝ ), ∫ (x: ℝ ), f x y :=
begin
  convert measure_theory.integral_integral_swap _,
  exact measure_theory.measure.regular.sigma_finite,
...
begin
  intros f g,
  have h₁:= multiplication_formula_step2 (λ y:ℝ,(λ x:ℝ, f.to_fun y * g.to_fun x * cexp (↑((-2) * real.pi * y * x) * I) )) _, 
  convert h₁,
  simp_rw fourier_integral_eq_integral_exp_smul g.to_fun _,
  ext1 x, rw [ mul_comm (f.to_fun x) _,← smul_eq_mul,← integral_smul_const],
  congr, ext1 y,
...
/-lemma step0 (f g: schwartz_map ℝ  ℂ ): measure_theory.integrable (function.uncurry (λ (y x : ℝ), f.to_fun y * g.to_fun x * cexp (↑((-2) * real.pi * y * x) * I))) (measure_theory.measure_space.volume.prod measure_theory.measure_space.volume):=
begin
  simp only [neg_mul, of_real_neg, of_real_mul, of_real_bit0, of_real_one],
  sorry,
end-/
...
/-lemma integrable_exp_mul_schwartz {x:ℝ} {f: schwartz_map ℝ ℂ}:
measure_theory.integrable (λ (v : ℝ), (λ (w : ℝ), -(2 * ↑real.pi * ↑v * I) * cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x) measure_theory.measure_space.volume:=
begin
  dsimp, simp_rw ← neg_mul,
  rw functions_equal_2,
  refine measure_theory.integrable.const_mul _ _,
  rw functions_equal_1,
  refine measure_theory.integrable.const_mul _ _,
  dsimp,
  convert integrable_mul_schwartz_map _ (λ (v : ℝ), cexp ((-2) * ↑real.pi * ↑v * ↑x * I)) (integrable_schwartz_map (schwartz_mul_x_pow 1 f)) _ _,
  ext1, congr, rw pow_one,
  have h₂:=cexp_bound x,
  obtain ⟨C,h₂⟩:=h₂,
  use C,
  intro x, specialize h₂ x,
...
/-
lemma dmimg  (a b: ℝ→ℝ  ) {i:ℕ}  (C₁ C₂: ℝ  ): ∫ (x : ℝ) in a i..b i, (C₁ + C₂) / (1 + ‖x‖ ^ 2) = (∫ (x : ℝ) in a i..b i, (1 + ‖x‖ ^ 2)⁻¹) • (C₁ + C₂):=
begin
  rw ← interval_integral.integral_smul_const,
  congr, ext1,
  rw [div_eq_mul_one_div, mul_comm, ← smul_eq_mul,inv_eq_one_div],
end

lemma dmimg: (0:ℝ) ≤ real.pi/2 + 1:=
begin
  rw ←zero_add (0:ℝ), 
  refine add_le_add _ zero_le_one,
  refine (le_of_lt real.pi_div_two_pos),
...
lemma hello (f: schwartz_map ℝ ℂ) {x:ℝ} (l:filter ℝ ) (a b: ℝ→ℝ  ):∃ I:ℝ,  ∀ᶠ (i : ℝ ) in l, ∫ (x : ℝ) in a i..b i, ‖f.to_fun x‖ ≤ I:=
begin
  have h₁:= f.decay',
  have h₂:= f.decay',
  specialize h₁ 0,  specialize h₂ 2,
  ...
  obtain ⟨C₁,h₁⟩:=h₁,
  obtain ⟨C₂,h₂⟩:=h₂,
  set C₃:= max C₁ C₂ with hmax,
  use (C₁ + C₂),
  refine filter.eventually_of_forall _,
  intro i,
  specialize h₁ x,  specialize h₂ x,
  simp only [pow_zero, norm_iterated_fderiv_zero, one_mul] at h₁, simp only [norm_iterated_fderiv_zero] at h₂,
  have h₃: ‖f.to_fun x‖ ≤ max C₁ (C₂/‖x‖^2), 
    cases le_or_lt (‖x‖) 0,
    rw le_max_iff,  left,
    exact h₁,
    rw mul_comm at h₂,
...
/-lemma translationhello (h: ℝ) (f:ℝ → ℂ) (h₁:0<h): (((h)⁻¹:ℝ ):ℂ)  * ∫ (y : ℝ), f (y) = ∫ (y : ℝ),  f (y*h) := 
begin
  --have h₁: ∫ (y : ℝ),  f y
  refine tendsto_nhds_unique _ n_top_integral, 
  have h₁:  tendsto (λ (n : ℝ), ∫ (v : ℝ) in -n..n, f (v)) at_top (nhds ( ∫ (y : ℝ), f y)),
    refine n_top_integral,
  --have h₁: ∀ n:ℝ, h • ∫ (x : ℝ) in -n..n, f (h * x) = ∫ (x : ℝ) in h * -n..h * n, f x,
  --  intro n, refine interval_integral.smul_integral_comp_mul_left _ _,
  rw metric.tendsto_at_top,
  rw metric.tendsto_at_top at h₁,
  intros ε hε,
  specialize h₁ (ε / ‖(h)⁻¹‖), specialize h₁ _, sorry,
  obtain ⟨N,h₁⟩:=h₁, 
  use N,
  intros n hn,
  specialize h₁ n, specialize h₁ hn,
  rw dist_eq_norm,
  rw interval_integral.integral_comp_mul_right _ _,
  have h₂: ‖(h⁻¹ • ∫ (x : ℝ) in -n * h..n * h, f x) - ↑h⁻¹ * ∫ (y : ℝ), f y‖ =‖(h:ℂ)⁻¹‖ * ‖ (∫ (x : ℝ) in -n * h..n * h, f x) - (∫ (y : ℝ), f y)‖,
    rw ←norm_mul _ _, congr, rw mul_sub, congr, sorry, norm_cast,
  rw h₂,
  rw mul_comm,
  rw ← lt_div_iff  _,
  rw smul_eq_mul,
  rw ← interval_integral.smul_integral_comp_mul_right,
  rw interval_integral.smul_integral_comp_mul_left,
  --have h₂:  tendsto (λ (n : ℝ),  ∫ (x : ℝ) in h * -n..h * n, f x ) at_top (nhds (∫ (x : ℝ), f x)),
  --rw metric.tendsto_at_top_nhds,
  --rw interval_integral.smul_integral_comp_mul_left,
end-/
...
lemma leibniz_rule_induction2 {f: schwartz_map ℝ ℂ} {n:ℕ}: 
∀ x:ℝ, deriv (iterated_deriv n (λ (w : ℝ), ∫ (v : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v)) x =  deriv (∫ (v : ℝ), iterated_deriv n (λ (w : ℝ), cexp (-(2 * ↑real.pi * ↑v * ↑w * I)) * f.to_fun v) x) x :=
begin
  induction n with n hn, 
  {intro x,
  simp only [iterated_deriv_zero],},
